...
import {listPlayers, listSchedules, listTeams} from '../graphql/queries';
...
const ChampModal = props => {
  ...
  //   return null;
  // } else {
  // const avatar1 = champPlayers[0];
  // const name1 = champPlayers[1];
  // const avatar2 = champPlayers[2];
  // const name2 = champPlayers[3];
...
const StandingsScreen = ({navigation, route}) => {
      ...
      if (option.isPlayoff === true) {
        ...
        if (isfinal !== false) {
          ...
          if (finalPoint[0] === 2 || finalPoint[1] === 2) {
            await getChamp(finalPoint, isfinal, homeID, awayID);
          }
        }
  ...
  const getChamp = React.useCallback(
    async (finalPoint, finalData, homeID, awayID) => {
      console.log(`finalPoint`, finalPoint);
      console.log(`finalData`, finalData);
      let champData;
      if (finalPoint[0] === 2) {
        // champImages = finalData[0];
        // champNames = finalData[1];
        setChampImage(finalData[0]);
        setChampName(finalData[1]);
        await getChampData(homeID);
      } else {
        // champImages = finalData[2];
        // champNames = finalData[3];
        setChampImage(finalData[2]);
        setChampName(finalData[3]);
        await getChampData(awayID);
      }
      // setChampPlayers(champData);
      // return champData;
    },
    [],
  );
  const getChampData = React.useCallback(async ID => {
    try {
  ...
  const fetchFinalSchedule = React.useCallback(async lgID => {
    try {
      ...
      return [finalSchedules, homeID, awayID];
    } catch (err) {
